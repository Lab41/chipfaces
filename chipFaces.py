...
def rotate_point(pos, img, angle):
    ...
    newx = x * cos(radians(angle)) + y * \
        sin(radians(angle)) + img.shape[1] * 0.4
    newy = -x * sin(radians(angle)) + y * \
        cos(radians(angle)) + img.shape[0] * 0.4
...
def process(in_dir, out_dir):
    ...
    for work_item in work_items:
        ...
        for x, y, w, h in detected[-1:]:
            ...
            roi = img[y:y + h, x:x + w]
            out_filename = '{0}-{1}-{2}-{3}-{4}.png'.format(
                work_item, x, y, x + w, y + h)
